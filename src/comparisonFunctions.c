...
int readHashEntry(hashentry *h, FILE *f, uint64_t freqThr)
{
	do
	{
		...
		{
			if (ferror(f))
				terror("Error reading hash entry from the file");
		}
						  ...
						  int fAst)
{
	...
	*n = 0;
	...
	if (!fAst)
		while ((c = buffered_fgetc(seq, &i, &r, f)) != '>' && (!feof(f) || (feof(f) && i < r)))
			; //start seq
	if (feof(f) && i >= r)
		return 0;
...
void writeFragmentRaw(struct FragFile *frag, FILE *f){
	fwrite(frag, sizeof(struct FragFile), 1, f);
}
...
void readFragmentRaw(struct FragFile *frag, FILE *f){
	if (fread(frag, sizeof(struct FragFile), 1, f) != 1){
		if (feof(f))
			return;
		terror("Error reading the HSP struct FragFile ");
	}
}
