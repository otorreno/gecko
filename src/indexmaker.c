...
int main(int ac, char **av) {
	...
	if ((fOut=fopen64(av[2],"wb"))==NULL) terror("Could not open output file");
    ...
    if ((RR =(struct rIndex2*) calloc(nReads,sizeof(struct rIndex2)))==NULL)
    	terror("memory for ordering Reads");

    if (( positions = (uint64_t*) calloc(nReads,sizeof(uint64_t)))==NULL)
    	terror("memory for ordering Reads--positions");


	if ((f=fopen64(av[2],"rb"))==NULL)
		terror("opening Reads.INDEX file");

 	for (i=0; i<nReads; i++){
	    if(0 < fread(&RR[i],sizeof(struct rIndex2),1,f)){
	    	positions[i]=i;	
	    } 
	...
	QsortRead(RR,0,nReads-1);

	// store ordered

	sprintf(tmp, "%s.sorted",av[2]);
	if ((fOut=fopen(tmp,"wb"))==NULL)
		terror("opening Reads.INDEX file ordered");
 	for (i=0; i<nReads; i++){
	    fwrite(&RR[i],sizeof(struct rIndex2),1,fOut); 
	}
	fclose(fOut);
